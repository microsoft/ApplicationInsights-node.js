...
function waitForOk(url, tries) {
    return getOk(url).then(ok => {
        if (!ok && (tries || 0) < 20) {
            Utils.Logger.info("Waiting for TestApp...");
...
function sendConfiguration(configuration) {
    Utils.Logger.info("Configuring TestApp...");
...
module.exports.startConnection = (configuration) => {
    return Utils.Logger.enterSubunit("Connecting to TestApp")
        .then(() => waitForOk(Config.TestAppAddress))
        .then(() => sendConfiguration(configuration))
        .then(() => Utils.Logger.exitSubunit());
}

module.exports.closeConnection = () => {
    return Utils.Logger.enterSubunit("Disconnecting from TestApp")
        .then(() => waitForOk(Config.TestAppAddress  + "/_close"))
        .then(() => Utils.Logger.exitSubunit());
}

module.exports.runTest = (testPath, silent) => {
    let promise = silent ? Promise.resolve() : Utils.Logger.enterSubunit("Running test " + testPath + "...");
    return promise
        .then(() => Utils.HTTP.get(Config.TestAppAddress + testPath))
        .then(() => !silent && Utils.Logger.exitSubunit());
}