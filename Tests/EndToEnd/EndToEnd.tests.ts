...
import { CorrelationContextManager } from "../../src/autoCollection/correlationContextManager";
import * as Constants from "../../src/declarations/Constants";
import * as Contracts from "../../src/declarations/Contracts";
import { HeartBeat } from "../../src/autoCollection/heartBeat";
...
describe("EndToEnd", () => {
  ...
  before(() => {
    ...
    interceptor = nock(Constants.DEFAULT_BREEZE_ENDPOINT).post("/v2.1/track", (body: string) => {
      return true;
    });
  ...
  describe("Basic usage", () => {
    ...
    it("should collect http request telemetry", (done) => {
      ...
      sandbox
        ...
        .callsFake((callback: (req: http.ServerRequest, res: http.ServerResponse) => void) => {
    ...
    it("should collect https request telemetry", (done) => {
      ...
      sandbox
        ...
        .callsFake(
          (options: any, callback: (req: http.ServerRequest, res: http.ServerResponse) => void) => {
            fakeHttpSrv.setCallback(callback);
            return fakeHttpSrv as any;
          }
        );
  ...
  describe("W3C mode", () => {
    ...
    it("should create and pass a traceparent header if w3c is enabled", (done) => {
      ...
      https.request(<any>"https://test.com", (c) => {
        eventEmitter.emit("response", {});
        assert.ok(
          (eventEmitter as any).headers.traceparent.match(/^00-[0-z]{32}-[0-z]{16}-[0-9a-f]{2}/g),
