...
describe("Library/InternalAzureLogger", () => {
    ...
    beforeEach(() => {
        ...
        sandbox.stub(global, "setInterval").callsFake(() => {
            return null;
        });
    });
    ...
    describe("Write to file", () => {
        ...
        it("should log message to file", (done) => {
            ...
            internalLogger["_storeToDisk"]("testMessage")
                .then(() => {
                    assert.ok(writeSpy.called);
                    assert.ok(
                        writeSpy.lastCall.args[0].toString().indexOf("applicationinsights.log") > 0
                    );
        ...
        it("should create backup file", (done) => {
            ...
            internalLogger["_storeToDisk"]("backupTestMessage")
                .then(() => {
                    ...
                    internalLogger["_storeToDisk"]("testMessage")
                        .then(() => {
                            internalLogger["_fileCleanupTask"]()
                                .then(() => {
                                    assert.ok(unlinkSpy.called);
                                    FileSystemHelper.readdirAsync(internalLogger["_tempDir"])
                                        .then((files) => {
