...
describe("#StandardMetricsHandler", () => {
    ...
    it("should observe instruments during collection", async () => {
        ...
        assert.equal(metrics[0].dataPoints.length, 1);
        assert.equal(metrics[0].dataPoints[0].value, 1);
        assert.equal(metrics[0].dataPoints[0].attributes["cloudRoleInstance"], 'testcloudRoleInstance');
        assert.equal(metrics[0].dataPoints[0].attributes["cloudRoleName"], 'testcloudRoleName');
        assert.equal(metrics[1].descriptor.name, StandardMetric.TRACES);
        assert.equal(metrics[1].descriptor.type, InstrumentType.OBSERVABLE_GAUGE);
        assert.equal(metrics[1].dataPoints.length, 1);
        assert.equal(metrics[1].dataPoints[0].value, 1);
        assert.equal(metrics[1].dataPoints[0].attributes["cloudRoleInstance"], 'testcloudRoleInstance');
        assert.equal(metrics[1].dataPoints[0].attributes["cloudRoleName"], 'testcloudRoleName');
    });
