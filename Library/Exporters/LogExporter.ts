...
import { AvailabilityTelemetry, TraceTelemetry, ExceptionTelemetry, EventTelemetry } from "../../Declarations/Contracts";
...
export class LogExporter extends BaseExporter {
    ...
    public async export(logs: AvailabilityTelemetry[] | TraceTelemetry[] | ExceptionTelemetry[] | EventTelemetry[],
        resultCallback: (result: ExportResult) => void): Promise<void> {
        diag.info(`Exporting ${logs.length} log(s). Converting to envelopes...`);
        const envelopes = logs.map((log) =>
            this._logToEnvelope(log, this._options.instrumentationKey)
        );
    ...
    */
     ...
     * @internal
     */
    private _logToEnvelope(telemetry: ExceptionTelemetry | AvailabilityTelemetry | TraceTelemetry | EventTelemetry
        , instrumentationKey: string): Envelope {
