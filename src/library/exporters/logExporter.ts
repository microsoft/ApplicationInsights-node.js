...
export class LogExporter extends BaseExporter {
    ...
    public async exportAvailability(
        logs: AvailabilityTelemetry[],
    ...
    public async exportException(
        logs: ExceptionTelemetry[],
        resultCallback: (result: ExportResult) => void
    ): Promise<void> {
        diag.info(`Exporting ${logs.length} log(s). Converting to envelopes...`);
        const envelopes = logs.map((log) =>
    ...
    public async exportPageView(
        logs: PageViewTelemetry[],
        resultCallback: (result: ExportResult) => void
    ): Promise<void> {
        diag.info(`Exporting ${logs.length} log(s). Converting to envelopes...`);
        const envelopes = logs.map((log) =>
