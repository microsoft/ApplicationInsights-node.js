...
export class AuthorizationHandler {
  ...
  constructor(credential: azureCore.TokenCredential) {
    ...
    let emptyPolicy: azureCore.RequestPolicy = {
      sendRequest(
        httpRequest: azureCore.WebResourceLike
      ): Promise<azureCore.HttpOperationResponse> {
        return null;
      },
    };
    this._azureTokenPolicy = azureCore
      .bearerTokenAuthenticationPolicy(credential, scopes)
      .create(emptyPolicy, new azureCore.RequestPolicyOptions());
  }

  /**
   * Applies the Bearer token to the request through the Authorization header.
   */
  public async addAuthorizationHeader(
