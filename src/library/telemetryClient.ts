...
import { Statsbeat } from "../autoCollection";
...
export class TelemetryClient {
  ...
  private _telemetryProcessors: {
    (envelope: Envelope, contextObjects: { [name: string]: any }): boolean;
  }[] = [];
  ...
  public addTelemetryProcessor(
    telemetryProcessor: (envelope: Envelope, contextObjects?: { [name: string]: any }) => boolean
  ) {
  ...
  private runTelemetryProcessors(
    ...
    if (accepted) {
      ...
      if (
        ...
        envelope.data.baseData.properties = Util.getInstance().validateStringMap(
          envelope.data.baseData.properties
        );
      }
    }
