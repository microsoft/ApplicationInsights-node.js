...
export class AutoCollectPerformance {
  ...
  private _trackMemory() {
    ...
    this._handler.trackMetric({
      metrics: [{ name: Constants.PerformanceCounter.PRIVATE_BYTES, value: usedMem }],
    });
    this._handler.trackMetric({
      metrics: [{ name: Constants.PerformanceCounter.AVAILABLE_BYTES, value: freeMem }],
    });
    // Only supported by quickpulse service
    if (this._enableLiveMetricsCounters) {
      this._handler.trackMetric({
        metrics: [{ name: Constants.QuickPulseCounter.COMMITTED_BYTES, value: committedMemory }],
      });
    }
  }
  ...
  private _trackNetwork() {
    ...
    if (elapsedMs > 0) {
      ...
      this._handler.trackMetric({
        metrics: [{ name: Constants.PerformanceCounter.REQUEST_RATE, value: requestsPerSec }],
      });
      ...
      if (this._enableLiveMetricsCounters) {
        this._handler.trackMetric({
          metrics: [
            { name: Constants.QuickPulseCounter.REQUEST_FAILURE_RATE, value: failedRequestsPerSec },
          ],
        });
      }
    }
  ...
  private _trackDependencyRate() {
    if (this._enableLiveMetricsCounters) {
      ...
      var intervalFailedDependencies =
        dependencies.totalFailedDependencyCount - lastDependencies.totalFailedDependencyCount || 0;
      var elapsedMs = dependencies.time - lastDependencies.time;
      var elapsedSeconds = elapsedMs / 1000;
      var averageDependencyExecutionTime =
        (this._intervalDependencyExecutionTime - this._lastIntervalDependencyExecutionTime) /
      ...
      if (elapsedMs > 0) {
        ...
        this._handler.trackMetric({
          metrics: [
            { name: Constants.QuickPulseCounter.DEPENDENCY_RATE, value: dependenciesPerSec },
          ],
        });
        this._handler.trackMetric({
          metrics: [
  ...
  private _trackExceptionRate() {
    if (this._enableLiveMetricsCounters) {
      ...
      if (elapsedMs > 0) {
        ...
        this._handler.trackMetric({
          metrics: [{ name: Constants.QuickPulseCounter.EXCEPTION_RATE, value: exceptionsPerSec }],
        });
      }
