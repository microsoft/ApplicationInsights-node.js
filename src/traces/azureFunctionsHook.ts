...
import { Disposable, PreInvocationContext } from "@azure/functions-core";
import { Context as AzureFnContext } from "@azure/functions";
import { context, propagation, ROOT_CONTEXT, Context as OpenTelemetryContext } from "@opentelemetry/api";
    ...
    private _addPreInvocationHook() {
        if (!this._preInvocationHook) {
            this._preInvocationHook = this._functionsCoreModule.registerHook("preInvocation", async (preInvocationContext: PreInvocationContext) => {
                ...
                let extractedContext: OpenTelemetryContext = null;
                try {
                    if (ctx.traceContext) {
